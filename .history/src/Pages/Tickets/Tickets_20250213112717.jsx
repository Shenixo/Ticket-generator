  ...
  const handleDownload = async () => {
    ...
    try {
      const canvas = await html2canvas(ticketElement, {
        scale: 2, // Keep scale 2 to avoid unnecessary stretching
        useCORS: true, // Allow external images
        allowTaint: true,
        backgroundColor: null, // Transparent background
        x: 0,  // Start from the left edge
        y: 0,  // Start from the top edge
        width: ticketElement.scrollWidth, // Full width of the ticket
        height: ticketElement.scrollHeight, // Full height of the ticket
      });
  
      const link = document.createElement("a");
      link.href = canvas.toDataURL("image/png");
      link.download = "ticket.png"; // Download filename
      link.click();
    } catch (error) {
      ...
      <Card className="big-card" background="#041E23">
        ...
        {isTicketReady && (
          <div className="button-container button-container-ticket">
            ...
            <Button type="submit" background="#24A0B5" color="#fff" onclickBtn={handleDownload}>
              Download Ticket
            </Button>
          </div>
        )}
