    ...
    <div className="field">
      ...
      {field === "input" && (
        <div>
          <input
            type={type}
            name={label_content.toLowerCase().replace(/\s+/g, "_")} // Convert label to a suitable name
            value={formData[label_content.toLowerCase().replace(/\s+/g, "_")] || ""}
            onChange={handleFormData}
            aria-describedby={`${label_content}-error`}
          />
          <div className={`error-message ${error[label_content.toLowerCase().replace(/\s+/g, "_")] ? "show" : ""}`}>
            <small role="alert">{error[label_content.toLowerCase().replace(/\s+/g, "_")]}</small>
          </div>
        </div>
      )}
      ...
      {field === "text_area" && (
        <div>
          <textarea
            ...
            className="text-area"
          ></textarea>
          <div className={`error-message ${error.special_request ? "show" : ""}`}>
      ...
      {field === "select" && (
        <>
          ...
          <div className={`error-message ${error.number_of_tickets ? "show" : ""}`}>
