      ...
      {field === "card" && (
        <Card className="select-ticket-type">
          {Object.entries(ticketType).map(([key, value], idx) => (
            <Card className={`ticket_type ${activeTab === key ? "active_tab" : null}` } onClickTab={() => setActiveTab(key)}  key={idx}>
              <div className="ticket_type-inner">
                <h3>{value.type}</h3>
                <p>{value.tickets_available} left!</p>
              </div>
              <Card className={`price-card ${activeTab === key ? "active_tab-_price" : null}` }>{value.price} </Card>
            </Card>
          ))}
        </Card>
      )}
