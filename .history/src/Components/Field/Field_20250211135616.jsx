...
const Field = ({ label_content, field }) => {
  ...
  return (
    <div className="field">
      ...
      {field === "input" && (
        <div>
          <input type="text" aria-describedby={`${label_content}-error`} />
          <div className={`error-message ${error.input_text ? "show" : ""}`}>
            <small>{error.input_text}</small>
          </div>
        </div>
      )}
      {field === "image-upload" && (
        <div>
          <input type="file" name="profile_upload" id="profile_upload" aria-describedby={`${label_content}-error`} />
          <div className={`error-message ${error.image_upload ? "show" : ""}`}>
            <smallrole="alert">{error.image_upload}</small>
          </div>
        </div>
      )}
