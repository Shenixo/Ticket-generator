      ...
      {field === "input" && (
        <div>
          ...
          <div className={`error-message ${error.input_text ? "show" : ""}`}>
            <small role="alert">{error.input_text}</small>
          </div>
        </div>
      )}
      {field === "image-upload" && (
        <div>
          <input className="image-upload" type="file" name="profile_upload" id="profile_upload" aria-describedby={`${label_content}-error`} />
