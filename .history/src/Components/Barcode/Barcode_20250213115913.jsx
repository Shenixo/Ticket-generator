  ...
  useEffect(() => {
    if (formData.number_of_tickets) {
      JsBarcode(barcodeRef.current, formData.number_of_tickets, {
        ...
        height: 40,
        displayValue: true,
        // Set the width dynamically to fill parent container
        margin: 0,
        background: 'transparent',
      });
      
      // Dynamically adjust the barcode to fill the container's width
      const svg = barcodeRef.current;
      const parentWidth = svg.parentElement.clientWidth;
      svg.setAttribute('width', parentWidth); // Adjust width of barcode
    }
