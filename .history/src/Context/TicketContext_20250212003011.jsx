...
const TicketContextProvider = ({ children }) => {
  ...
  const savedToProceed = localStorage.getItem("toProceed") === "true"; // Ensure boolean conversion
...
axios
    ...
    if (file) {
      ...
      try {
        ...
        if (response.data.secure_url) {
          ...
          setError((prev) => ({ ...prev, profile_upload: "Failed to upload image" }));
        }
      } catch (error) {
        console.error("Upload Error:", error);
        setError((prev) => ({ ...prev, profile_upload: "Image upload failed" }));
      }
    }
  };
  ...
  useEffect(() => {
    localStorage.setItem("toProceed", toProceed); // Persist toProceed state
  }, [toProceed]);
